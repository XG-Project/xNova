...
function PlanetResourceUpdate($CurrentUser, &$CurrentPlanet, $UpdateTime, $Simul = FALSE)
{
	...
	if ($CurrentPlanet['planet_type'] != 3)
	{
		foreach ($reslist['prod'] as $ProdID)
		{
			...
			foreach ($rList as $rname => $rdata)
			{
				$cProd			= eval($ProdGrid[$ProdID]['formule'][$rname]);// PRODUCTION FORMULA
				$CurrentPlanet[$rname.'_perhour']	+= Production::current_production(Production::production_amount($cProd, $geologe_boost), $post_porcent);
			}
...
/* End of file PlanetResourceUpdate.php */
/* Location: ./includes/functions/PlanetResourceUpdate.php */